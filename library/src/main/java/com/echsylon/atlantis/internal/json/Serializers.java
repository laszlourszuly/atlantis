...
class Serializers {
    ...
    // This is a marker class preventing eternal circular parse loops in gson.
    private static final class _Response extends Response {
    }

    // This is a marker class preventing eternal circular parse loops in gson.
    private static final class _Request extends Request {
    }
    ...
    private static Configuration deserializeConfiguration(JsonElement json, JsonDeserializationContext context) {
        // Remove the 'requestFilter' attribute as it's a string in the JSON.
        // The Configuration object expects it to be a Java object. We'll later
        // parse the removed string and set the object field manually.
    ...
    private static Response deserializeResponse(JsonElement json, JsonDeserializationContext context) {
        JsonObject jsonObject = prepareHeadersForObject(json);
        return context.deserialize(jsonObject, _Response.class);
    }
    ...
    private static JsonElement serializeResponse(Response response, JsonSerializationContext context) {
        return context.serialize(response, _Response.class);
    }
    ...
    private static Request deserializeRequest(JsonElement json, JsonDeserializationContext context) {
        // Remove the 'responseFilter' attribute as it's a string in the JSON.
        // The Request object expects it to be a Java object. We'll later parse
        // the removed string and set the object field manually.
        JsonObject jsonObject = prepareHeadersForObject(json);
        String filterClassName = jsonObject.has("responseFilter") ?
                jsonObject.remove("responseFilter").getAsString() :
                null;

        Request request = context.deserialize(jsonObject, _Request.class);
        if (Utils.isEmpty(filterClassName))
            return request;
    ...
    private static JsonElement serializeRequest(Request request, JsonSerializationContext context) {
        JsonElement jsonElement = context.serialize(request, _Request.class);
        if (jsonElement == null)
            return null;
    ...
    private static JsonObject prepareHeadersForObject(JsonElement json) {
        JsonObject jsonObject = json.getAsJsonObject();
